...
void Render::Vulkan::Tool::GenerateMipMap(Render::Vulkan::VulkanDevice* device,
                                          VkCommandBuffer cmd,
                                          VkQueue queue, VkImage image, VkFormat format, int32_t texWidth, int32_t texHeight, uint32_t mipLevels, uint32_t layerCount)
{
    ...
    for (uint32_t layer = 0; layer < layerCount; layer++)
    {
        ...
        for (uint32_t i = 1; i < mipLevels; i++) {
            ...
            // 第一步：将mip i-1从TRANSFER_DST转换为TRANSFER_SRC
            ...
            barrier.subresourceRange.baseMipLevel = i - 1;  // 源mip层
            barrier.subresourceRange.levelCount = 1;
            barrier.subresourceRange.baseArrayLayer = layer;
            barrier.subresourceRange.layerCount = 1;
            barrier.oldLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;  // 当前布局
            barrier.newLayout = VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL;  // 目标布局
            ...
            // 第二步：将mip i-1从TRANSFER_SRC转换为SHADER_READ_ONLY
        ...
        finalBarrier.subresourceRange.baseMipLevel = mipLevels - 1;  // 最后一个mip层
        finalBarrier.subresourceRange.levelCount = 1;
        finalBarrier.subresourceRange.baseArrayLayer = layer;
        finalBarrier.subresourceRange.layerCount = 1;
        finalBarrier.oldLayout = VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL;  // 它一直是DST
